...
def get_token_auth_header():
    ...
    if auth_header.split()[0].lower() != 'bearer' or len(
            auth_header.split()) != 2:
        raise AuthError({
            'code': 'invalid_auth_header',
            'description': 'Header must be in "bearer" token format.'
        }, 401)
...
def check_permissions(permission, payload):
    if not payload['permissions']:
        raise AuthError({
            ...
            'description': 'Token doesn\'t contain permissions payload'
        }, 401)
