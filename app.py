...
def create_app(test_config=None):
  ...
  def post_bookings(auth_payload):
    ...
    try:
      ...
      user = Developer.query.filter(Developer.username==auth_sub).one_or_none()
      if user is None:
        abort(404)

      # fail if the resource is booked already
      booking_for_same_resource = Booking.query.filter(Booking.resource_id==body.get("resource_id")).one_or_none()
      ...
      new_booking.expected_duration_hours = body.get("expected_duration_hours")
  ...
  @requires_auth() # set no explicit permission here because finer control is required here
  def delete_booking(auth_payload, booking_id):
    ...
    if not (
      (
        ...
        booking.developer_id==user.id and 'delete:own_bookings' in auth_payload['permissions']
      )
    or (
